...
subroutine get_helm_table(temp,den,f,fd,ft,fdd,ftt,fdt,fddt,fdtt,fddtt)
   ...
   interface
      function dfridr1(f,t,d,h,err)
           implicit none
           external f
           double precision :: t,d,h,err, dfridr1
      end function
   
      function dfridr2(f,t,d,h,err)
           implicit none
           external f
...
function dfridr1(func,x,y,h,err)
    implicit none
    INTEGER NTAB
    double precision dfridr1,err,h,x,y,CON,CON2,BIG,SAFE
    ...
    enddo 
    return
end function dfridr1

function dfridr2(func,x,y,h,err)
    implicit none
    INTEGER NTAB
    double precision dfridr2,err,h,x,y,CON,CON2,BIG,SAFE
    PARAMETER (CON=1.4d0,CON2=CON*CON,BIG=1.d30,NTAB=10,SAFE=2.0d0)
    EXTERNAL func
    INTERFACE
       FUNCTION func(x,y)
...
#if 0
function dfridr3(func,x,y,h,k,err)
    implicit none
    INTEGER NTAB
    double precision dfridr2,err,h,k,x,y,CON,CON2,BIG,SAFE
    PARAMETER (CON=1.4d0,CON2=CON*CON,BIG=1.d30,NTAB=10,SAFE=2.0d0)
    EXTERNAL func
    INTERFACE
       FUNCTION func(x,y)
    ...
    !NTAB. Return when error is SAFE worse than the best so far.
    INTEGER i,j
    double precision errt,fac,hh,kk,a(NTAB,NTAB)
    hh=h
    kk=k
    a(1,1) = (func(x,y+hh) - 2.0d0*func(x,y) + func(x,y-hh)) / (hh*hh)
    err=BIG
...
! end of pipeline loop
      ...
      enddo
        ...
        e121 = (eele_row(1) + epos_row(1))
        s121 = (sele_row(1) + spos_row(1))
        p121 = (pele_row(1) + ppos_row(1))
        t121 = temp
        d121 = den
...
! local variables
      ...
      !sneo: free energy derivatives
      double precision p, s, e, d, t, f, fd, ft, fdd, ftt, &
              fdt, fddt, fdtt, fddtt, err
      external  dfree1, sine, helm_free!square!, dfridr1  
      double precision dpdf, dpdfd, dpdft, dpdfdt
      double precision ef, efd, eft, efdt
      double precision deriv

      !interface
      !function dfridr1(f,x,y,h,err)
      !    implicit none
      !    double precision, intent(in) :: x,y,h,err
      !    external :: f
      !    double precision :: dfridr1
      !end function dfridr1
      !end interface
! popular formats
...
! electron chemical potential, positron chemical potential and its derivatives
      ...
      !fdtt = dfridr1(helm_free,t,d,1.0d0,err)
      write(6,*) "required fdtt = ",fdtt
      write(6,*) "with error = ",err
      !write(6,03) 'fdtt =', fdtt

      !deriv = dfridr1(sine,0.0d0,0.0d0,1.0d0,err)
      !write(6,*) "derivative of sin at 0 = ",deriv 
      !write(6,*) "with error = ",err
! end of pipeline loop
      enddo
